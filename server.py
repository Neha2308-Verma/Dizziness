...
EYE_THRESHOLD = 0.01  # Reduced threshold for eye detection
MIN_CONSECUTIVE_FRAMES = 3  # Increased minimum frames for state change
MAX_EYE_AREA = 0.03  # Maximum eye area ratio to face
...
def detect_eyes(gray, face):
    ...
    # Define eye region of interest (upper half of face)
    roi_gray = roi_gray[int(h/6):int(h/2), :]
    ...
    # More granular eye state detection
    if len(eyes) < 2:  # If less than 2 eyes detected
        return 0  # Closed
    elif relative_eye_size < EYE_THRESHOLD:
        return 0  # Closed
    elif relative_eye_size > MAX_EYE_AREA:
        return 2  # Open - large eyes detected
    elif relative_eye_size < EYE_THRESHOLD * 2:
        return 1  # Drowsy
    else:
        return 2  # Open
